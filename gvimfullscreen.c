...
LONG __declspec(dllexport) ToggleFullScreen()
{
    ...
    if (hTop != NULL && hTextArea != NULL)
    {
        ...
        if ( GetWindowLong(hTop, GWL_STYLE) & WS_CAPTION )
        {
            ...
            sprintf(p, "gVim_Position=%ld\t%ld\t%ld\t%ld\t%ld", rc.left, rc.top, rc.right-rc.left, rc.bottom-rc.top, z);
            putenv(p);
        ...
        else
        {
            ...
            {
                sscanf(p, "%ld\t%ld\t%ld\t%ld\t%ld", &L, &R, &W, &H, &Z);
...
LONG __declspec(dllexport) SetAlpha(LONG nTrans)
{
    HWND hTop = NULL;
...
LONG __declspec(dllexport) EnableTopMost(LONG bEnable)
{
    HWND hTop = NULL;
    DWORD dwThreadID;
